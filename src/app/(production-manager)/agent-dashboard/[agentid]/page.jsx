...
export default function AgentDashboardPage() {
  ...
  const {
    ...
    currentUser?.id && agentid
      ? `/api/purchaseOrder?managerId=${currentUser.id}&agentId=${agentid}`
      : null,
    fetcher,
    {
      revalidateOnFocus: true, // refresh when tab gains focus
      dedupingInterval: 60 * 1000, // prevent refetch within 1 min
    }
  );
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 className="font-bold text-3xl break-words">
          {purchaseOrders?.[0]?.agentName || "Agent Dashboard"}
        </h2>
